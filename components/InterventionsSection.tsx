    ...
    <div>
      ...
      <div className="space-y-3">
        {interventions.map((item, i) => {
          ...
          return (
            <div key={i} className="border border-gray-200 rounded-xl bg-white hover:shadow-sm transition-shadow animate-fade-in-up overflow-hidden">
              {/* Card header row */}
              <div className="flex items-start gap-3 p-4 pb-3">
                {/* Priority number badge */}
                <div className="shrink-0 w-7 h-7 rounded-lg bg-gray-900 flex items-center justify-center mt-0.5">
                  <span className="text-xs font-bold text-white leading-none">{i + 1}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <p className="text-sm font-semibold text-gray-800 leading-snug">{item.intervention}</p>
              ...
              {item.implementation_guidance && (
                <div className="mx-4 mb-4 p-3 bg-gray-50 rounded-lg border border-gray-100">
